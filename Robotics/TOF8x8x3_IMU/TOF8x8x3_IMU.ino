...
//                                   id, address
Adafruit_BNO055 bno = Adafruit_BNO055(55, i2cImuAddr, &Wire);
// rate to read BNO055 sensor and send data over serial port
int bnoRateHz = 50;
uint32_t bnoLastMillis = 0;
...
void setup()
{
  ...
  Serial.begin(115200);
  delay(1000);
  Serial.println("VL53L5CX TOF Imager and BNO055 IMU");
...
void loop()
{
  // Check for serial config string
  ...
  // TOF8x8x3 poll until all 3 sensors have data ready
Serial.println("Before getSensorData");
  if(!dataRdy[0]) dataRdy[0] = getSensorData(&myImagerL, 0);
Serial.println("After getSensorData0");
  if(!dataRdy[1]) dataRdy[1] = getSensorData(&myImagerF, 1);
Serial.println("After getSensorData1");
  if(!dataRdy[2]) dataRdy[2] = getSensorData(&myImagerR, 2);
Serial.println("After getSensorData2");
  // Send frame of TOF sensor data when all 3 sets are ready
  ...
  // if((nowMillis - bnoLastMillis) >= (1000/bnoRateHz)) {
  //   bnoLastMillis = nowMillis;

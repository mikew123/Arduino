...
void loop(void)
{
  // Send calibration status 1/sec
  if((millis()-cal_last_millis)>=1000) {
    cal_last_millis = millis();
  ...
  //while((millis()-imu_last_millis)<BNO055_SAMPLERATE_DELAY_MS);
  if((millis()-imu_last_millis)>=BNO055_SAMPLERATE_DELAY_MS) {
    imu_last_millis = millis();

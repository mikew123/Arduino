...
float secPerCm;
  ...
  default:
    ...
    else             secPerCm = (1.05/speed)*(1.0/25);
    break;
  }
  return(secPerCm);
}
...
float secPerCm;
  ...
  case fwd:
    ...
    else             secPerCm = (0.90/speed)*(1.0/25);
    break;
  case rev:
  default:
    if(speed == 0.5) secPerCm = (3.0/speed)*(1.0/25);
    else             secPerCm = (1.5/speed)*(1.0/25);
    break;
  }
  return(secPerCm);
}
...
void openClaws()
{
  // set claw servos to open position
  int rClawUs = clawOpenUsec;
...
void moveY(int yCm, bool senseGoal = false, int *robotY_p = NULL)
{
  float speed;
  int motion;  
  int runTimeMs;
...
Serial.print("Goal X L,R = "); Serial.print(wallLX);Serial.print(", "); Serial.println(wallRX);
      if(wallRX<80 && wallLX>-80)
      {
Serial.print("Goal detected, stop movement");
        if(robotY_p!=NULL) *robotY_p = wallAbsY; // The goal is at Y wall coordinates        
...
void moveXY(int xCm, int yCm, bool senseGoal = false, int *robotY = NULL)
{
  moveX(xCm);
  moveY(yCm, senseGoal, robotY);
  // TODO: Measure actual time and trim amount of XY movement?

  // Stop movement
  for (int wheel=0;wheel<4;wheel++)
    wheels.WheelDrive(wheel, stop, 0, sensors.heading);

}


void moveYX(int xCm, int yCm)
{
  moveY(yCm);
  moveX(xCm);
  // TODO: Measure actual time and trim amount of XY movement?
...
Serial.print(robotScanXY[robotScanXYidx].Y);Serial.print("\n");
    ...
    moveXY(movX, movY);
    robotY += movY; // estimateY
    if(measRobotX(robotX) == false) robotX += movX;
  ...
  case loopState_Scan4Can:  
    ...
    sensors.calibrateAG = false; // stop calibration after scan
...
Serial.print("canYcm "); Serial.print(canYcm); Serial.print("\n");
      ...
      if(canDist >= 5) loopState = loopState_Move2Can;
  ...
  case loopState_UpdateXY:
    // update robotX if available, else estimate X
    // estimate robotY
    robotY += canYcm;
    if(measRobotX(robotX) == false) robotX += canXcm;
Serial.print("Robot XY = "); Serial.print(robotX); Serial.print(","); Serial.println(robotY);
  ...
  case loopState_Can2Goal:
    ...
    goalYcm = wallAbsY - robotY + 20; // try to go past goal
    // detect the goal walls while moving to goal and set robotY coordinate
    moveXY(goalXcm, goalYcm, true, &robotY); 
    robotY = wallAbsY; // stopped when goal (wall Y) detected 
Serial.print("Robot XY = "); Serial.print(robotX); Serial.print(","); Serial.println(robotY);
    // go 10cm into goal area to ensure can is in
    moveXY(0, 10);
    robotY += 10;

...
void ObjectDetect::detectObjects(bool scanEnd, int rl, int scanDeg, int scanDist, int x, int y)
{
  ...
  int wallLimX = wallLocX - robotLocX - 25; // 25 is for measure error
  int wallLimY = wallLocY - robotLocY - 25; // 25 is for measure error
  ...
  case detObj_ObjStart: // look for start of object in scan
    if(scanDist<95 && (scanDeg>=0?x<wallLimX:x>-wallLimX) && y<wallLimY)

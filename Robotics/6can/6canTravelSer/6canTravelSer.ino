...
void openClaws(int openPct = 100)
{
  ...
  // TODO: open slower? cans in goal get knocked down!!!
...
//  if(clawOpenedUsec != clawCloseUsec)
//  {
...
void moveX(int &xCm, bool lidar = false)
{
  ...
  busV = ina219.getBusVoltage(); delay(1);
  runTimeMs = ((motion==right)?+1:-1)*(1000L*xCm)*secPerCmX(busV, motion, speed);
...
// TODO: goalSensorDistMin ?
  ...
  float busV = ina219.getBusVoltage(); delay(1);
  runTimeMs = (1000L*yCm)*secPerCmY(busV, motion, speed);

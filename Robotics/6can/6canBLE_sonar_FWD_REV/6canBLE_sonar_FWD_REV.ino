...
// MRW 4/29/2023 Tweaked DMS settings
// MRW 5/3/2023 Tweaking objdet 
// MRW 5/6/2023 Seperate sonar trigger signals since 3.3V sonars interfere
// MRW 5/7/2023 Add FWD REV run for DMS, use CW=15 command
...
//******************************************************
//  It was going against left wall at DMS
...
void initArenaDms()
{
  wallAbsX  = 107; // wall X offset from center any Y=0, walls parallel
  wallAbsY  = 152; // goals are at center of these walls at X=0
  robStartX = 0;   // robot starting location X,Y = (0,-50)
  robStartY =-106; 
  canCount  =  0;
}
...
float calSecPerCmFwd2p0_A  =  0.296595;
float calSecPerCmFwd2p0_B  = -0.081120;
float calSecPerCmFwd2p0_C  =  0.005977;

float calSecPerCmRev2p0_A  =  0.129455;
float calSecPerCmRev2p0_B  = -0.029742;
float calSecPerCmRev2p0_C  =  0.002044;
...
enum {
  ...
  ble_left  = 3,
  ble_right = 4,
  ble_fast  = 13, // use for 2X FWD-REV calibration
  ble_cw    = 15, // use for DMS fwd-rev course
  ble_6canHome = 20,
  ble_6canDms  = 21,
...
// if(serialOK) {Serial.print("ctrlDataRW = ");Serial.println(value);}
          switch (value)
          {
            ...
            case ble_fast: // fwd-rev 2X calibration
              if(serialOK) Serial.println("CW");
              initArenaHome();
              manBleCmd = ble_fast;
              bleSendState("2X FR cal");
              break;

            case ble_cw: // fwd-rev DMS course
              if(serialOK) Serial.println("CW");
              initArenaDms();
              manBleCmd = ble_cw;
              bleSendState("DMS QT");
...
float secPerCm;
  ...
  case fwd:
    if(speed > 1.5)  secPerCm = calSecPerCmFwd2p0_A + (volts * calSecPerCmFwd2p0_B) + (volts*volts * calSecPerCmFwd2p0_C);
    if(speed > 0.75) secPerCm = calSecPerCmFwd1p0_A + (volts * calSecPerCmFwd1p0_B) + (volts*volts * calSecPerCmFwd1p0_C);
...
secPerCm *= (45.0/50.0);
  ...
  case rev:
  default:
    if(speed > 1.5)  secPerCm = calSecPerCmRev2p0_A + (volts * calSecPerCmRev2p0_B) + (volts*volts * calSecPerCmRev2p0_C);
    if(speed > 0.75) secPerCm = calSecPerCmRev1p0_A + (volts * calSecPerCmRev1p0_B) + (volts*volts * calSecPerCmRev1p0_C);
...
bool  measRobotX(int &robotX)
{
  ...
  int wallRX, wallLX;
  objdet.getSonarRL(wallRX, wallLX);
  wallLX = -wallLX; // negative X direction
  ...
  { // it is within 10%
    ...
    robotX = -(wallLX + wallRX); // center of measurements
    validMeas = true;
  }
...
stopCm=-1;
  ...
  if(lidar == true) objdet.positionRangeSensors(objdet.scanDegStart);
...
Serial.print("millis = "); Serial.print(millis() - startMillis); Serial.print("\n");
    ...
    if(lidar == true)
    {
      if((lastProx < 30)&&((proximity - lastProx) > 5))       
...
stopCm=-1;
  ...
  { // set sensors to detect can right in front, focus toward middle
    objdet.positionRangeSensors(objdet.scanDegStart);
  }
  else if (stopCm > 0)
...
Serial.print("millis = "); Serial.print(millis() - startMillis); Serial.print("\n");
    ...
    // detect goal opening by sensor pointing up and detects board over goal opening
    else if(senseGoal == true)  
    {   
      objdet.getSonarRLT(cmR, cmL, cmT);
            ...
            , bool slow = false, bool avoidObstacle=false)
{
  ...
  {
    xCm1 = xCm;
    obstacleDetX = moveX(xCm1, lidar, slow, 15); // Sonar at 0
  ...
  {
    yCm2 = yCm - yCm1;
    obstacleDetY = moveY(yCm2, senseGoal, lidar, slow, 5); // sonar at +-90
    yCm3 = yCm - yCm1 - yCm2;
...
//  sonicSensors.sensorR(0); // start new sensor measurement
  startMillis = millis();
  millisElapsed = 0;
...
//Serial.print(">");
    ...
    objdet.getSonarRL(cmR, cmL, true);
...
//Serial.print(cmL);Serial.print(", ");Serial.print(cmR);Serial.print(", ");
...
//  objdet.initObjectDetect(serialOK, &pca9685, &sonicSensors, &bleSensors, wallAbsX, wallAbsY);
  objdet.initObjectDetect(serialOK, &pca9685, &sensorL, &sensorR, &sensorT
                                  , &bleSensors, wallAbsX, wallAbsY);
...
//******************************************************
//         It was going against the left wall at DMS

// Robot scan positions to use
#define numRobotScanXY 15
int robotScanXYidx = 0;
// home=0, dms=1
XY robotScanXY[2][numRobotScanXY] = 
{
  {
    {  0,-50}, {  0,  0}, {  0, 50}, {  0,-50}, {-90,-50}, 
    {-90,  0}, {-90, 50}, {  0,-50}, { 90,-50}, { 90,  0}, 
    { 90, 50}, {  0,-50}, {  0,  0}, {  0, 50}
  },
  {
    {  0,-106}, {  0, -50}, {  0,  0}, {  0, 50}, {  0, 75},
    {-85, -75}, {-85, -25}, {-85, 25}, {-85, 75}, {-85, 95},
    { 85, -75}, { 85, -25}, { 85, 25}, { 85, 75}, { 85, 95}
  }
};
...
// DPRG-DMS Quick Trip course
/*
Objective: The robot will travel between two zones that are approximately 12 feet apart, making
a round trip from one zone to the other and returning to the first zone. The robot must come to a
complete stop at the end of both legs of its journey. A lane between the two zones is defined
and the robot’s travel must remain within the lane’s boundaries. In the start and ending zones a
space of 6 inches is marked that represents the center of the starting zone. Additional
consideration is given to robots that place their center inside of the 6-inch space in each zone
while they are stopped in a zone.
*/
void bleQuickTripMode()
{
  int cmFwd = (12+1)*12*2.54;
  int cmRev = cmFwd+0;
  float stopSec = 1.0;
  float speed = 2.0;
...
//  float fwdSec = cmFwd*secPerCmY(busV, fwd, speed);
//  float revSec = cmRev*secPerCmY(busV, rev, speed);
  float fwdSec = 10.0;
  float revSec = 10.5;

  bleSensors.calibrateAG = false;

  while(millisElapsed < (1000*fwdSec))
  {   
    for (int wheel=0;wheel<4;wheel++)
        wheels.WheelDrive(wheel, fwd, speed, bleSensors.heading);
    millisElapsed = millis() - startMillis;
  }

  startMillis = millis();
  millisElapsed = 0;
  while(millisElapsed < (1000*stopSec))
  {   
    for (int wheel=0;wheel<4;wheel++)
  ...
  startMillis = millis();
  millisElapsed = 0;
  while(millisElapsed < (1000*revSec))
  {   
    for (int wheel=0;wheel<4;wheel++)
        wheels.WheelDrive(wheel, rev, speed, bleSensors.heading);
    millisElapsed = millis() - startMillis;
  }
...
if(serialOK) {
  Serial.println("loopState_Move2Can");
    ...
    if((abs(canDeg) <= 75) && (canYcm<5) && (abs(canXcm)<10)) 
    { // can in front move back and to the side to scan using lidar
      int yCm = canYcm -2;
      int xCm = (canXcm>=0)? -2:2;
      moveYX(yCm, xCm); // Can is close, move a bit toward back of can
      // estimate new can XY offset (deg dist not estimated)
      canXcm += xCm;
      canYcm += yCm;
      loopState = loopState_Move2CanLidar;
    }
    else if(abs(canDeg) > 85)
    { // backup before going to location, but stay behind a little
      int yCm = -15;
      int xCm = 0;
      moveYX(yCm, xCm);      
      xCm = canXcm;
      yCm = canYcm + 10;
      moveXY(xCm, yCm,0,0,0,1);// avoid obstacles sonar +-90 then 0
      loopState = loopState_Scan4Can;
    }
    else if(abs(canDeg) > 75)
    ...
    { // go to can location
      if(canYcm > 1) moveXY(canXcm, canYcm, 0, 0, 0, 1); // avoid obstacles  sonar +-90 then 0
      else           moveYX(canYcm -3, canXcm);
      loopState = loopState_Scan4Can;
    }

    // X location correction, sonar moves +-90
    // Is this causing all the extra sonar movement?
    //measRobotX(robotX);
...
if(serialOK) {
  ...
  Serial.print("canYcm "); Serial.print(canYcm); Serial.print("\n");
    ...
    // Move robot in X L or R until can center is detected using sonar at 0
    if(canXcm >= 0) 
    {
      moveXY(-10,0,false,false);
      moveXY(+30,10,false,true); // move at most X=30, Y=10 cm
    }
    else            
    {
      moveXY(+10,0,false,false);
      moveXY(-30,10,false,true);
    }

    // Check if can is in front
    objdet.positionRangeSensors(objdet.scanDegStart);
    delay(100);
    objdet.getSonarRL(cmR, cmL);
...
if(serialOK) {
  Serial.println("loopState_GrabCan");
    ...
    // TODO: Verify can was grabbed with sonar
    closeClaws();    
...
//loopState = loopState_Done; // DEBUG stop
  ...
  case loopState_Can2Goal:
    ...
    moveXY(goalXcm, goalYcm, true,0,0,1); // avoid obstacals
...
Serial.print("At goal robot XY = "); Serial.print(robotX); Serial.print(","); Serial.println(robotY);
    ...
    // backup out of the goal and measure the robotX coordinate and recenter
    moveXY(0, -35);

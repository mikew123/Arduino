...
// MRW 3/13/2023 detect works OK with new state machine and averaged degrees
    ...
    { // last scan point, combine RL data
      ...
      { // calc object location using both RL
        if(0){
          ...
          if(objDetVars[rl].objDegStart > objDetVars[rl?0:1].objDegStart) 
            objDetVars[rl].objDeg = objDetVars[rl].objDegStart -
              ((objDetVars[rl].objDegStart - objDetVars[rl?0:1].objDegStart)/2);
          else 
            objDetVars[rl].objDeg = objDetVars[rl?0:1].objDegStart -
              ((objDetVars[rl?0:1].objDegStart - objDetVars[rl].objDegStart)/2);
        } else {
...
//    else if(!objDetVars[rl].objScanEnd && possibleObj)
//    {
//      // continue scaning object
//      
//      break; // State detObj_ObjScan
//    }
    ...
    } // end if nonObjSeqCnt > 5 - end of object detected
    else

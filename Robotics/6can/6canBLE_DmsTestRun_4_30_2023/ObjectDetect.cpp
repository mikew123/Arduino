...
//      scanDeg += (scanMax2Min ? -scanDegInc : scanDegInc))
  for(scanDeg = scanDegFwd; (scanDeg <= scanDegRev); scanDeg += scanDegInc)
  {
...
void ObjectDetect::getSonarRL(int &cmR, int &cmL)
{
  int count = 0;
  sonicSensors_p->startAsync(0); 
  while (!sonicSensors_p->isFinished()) 
  {
    delay(25); 
  ...
  cmR = sonicSensors_p->getDist_cm(rightSensor) - 6; // right sensor
  cmL = sonicSensors_p->getDist_cm(leftSensor)  - 6; // left sensor
}

...
// Alternate way servo.writeMicroseconds(us) may aloow > 0-180
...
bool OperateClaws(int clawDesiredPct, int clawPeriodMs) {
  bool operating = true;
  float dt = 10.0; // 10 ms constant update period
  uint32_t now = millis();
  ...
  if (now >= clawNextTime) {
    ...
    // Convert percent to degrees and operate claws
//    int clawDeg = 180*clawCurrentPct/100;
//    myservoL.write(clawDeg);
//   myservoR.write(180-clawDeg);

    int clawUsecRange = clawUsecMaxL - clawUsecMinL;
    int clawUsec = clawUsecMinL + clawUsecRange*clawCurrentPct/100;
    myservoL.writeMicroseconds(clawUsec);

    clawUsecRange = clawUsecMaxR - clawUsecMinR;
    clawUsec = clawUsecMaxR - clawUsecRange*clawCurrentPct/100;
    myservoR.writeMicroseconds(clawUsec);
